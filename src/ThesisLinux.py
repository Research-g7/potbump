...
# Set up base paths for Linux
ROOT_DIR = Path(__file__).resolve().parent.parent
DATA_DIR = ROOT_DIR / "data"
MODELS_DIR = DATA_DIR / "models"
SOUNDS_DIR = DATA_DIR / "sounds"

# Create directories with Linux permissions
os.makedirs(MODELS_DIR, mode=0o755, exist_ok=True)
os.makedirs(SOUNDS_DIR, mode=0o755, exist_ok=True)
...
def read_gps():
    # Linux serial port naming
    port = "/dev/ttyUSB0"  # Common USB GPS device location
    baudrate = 9600
    try:
...
def initialize_camera():
    try:
        # Try different video devices in Linux
        for device in range(10):
            cap = cv2.VideoCapture(device)
            if cap.isOpened():
                print(f"Camera initialized on /dev/video{device}")
                
                # Linux-compatible camera settings
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)  # Reduced from 4K for better performance
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
                cap.set(cv2.CAP_PROP_FPS, 30)
                cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                
                # Use V4L2 codec if available
...
if __name__ == '__main__':
    try:
        # Initialize camera using the new function
        ...
        qr_path = os.path.join(DATA_DIR, 'camera_stream_qr.png')
        qr_img.save(qr_path)
